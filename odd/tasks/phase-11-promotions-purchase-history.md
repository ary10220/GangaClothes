...
- Review evidence: RDD was `on (decided by global)`. `gentle-ai review assess` classified the consolidated candidate as `medium` with `review_due_reason=slice_budget_reached` (22 files, 1856 changed lines). The person declined review for this candidate; the exact decline invocation returned `action: declined` and `consent: declined_this_candidate`. No receipt or acknowledgement was produced, no review approval is claimed, and delivery remains under ordinary policy. Future reviews remain active.
